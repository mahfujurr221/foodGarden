<script>
    ...
    function domPrepend(product = null, index = null) {
        ...
        let dom = `
              <tr class="background_gray">
                ...
                <td style="width:150px;display:none">
                  <input readonly type="hidden" value="${product.cost}" class="form-control rate border_none background_gray text-center" name="rate[${product.id}]" />
                  <select class="form-control" name="damage_type[${product.id}]">
                    <option value="product">Product</option>
                    <option value="taka">Taka</option>
                    </select>
                </td>
                <td style="width:150px">
                  <input type="text" readonly name="sub_total[${product.id}]" class="form-control sub_total border_none background_gray text-center" value="${product.cost}"/>
                  <input type="hidden"  name="total_amount" class="form-control total_amount" value="${product.cost}"/>
                </td>
        ...
        $(".increase-qty").click(function() {
            var inputField = $(this).siblings(".main_qty");
            inputField.val(parseInt(inputField.val()) + 1);
            handleQuantityChange(inputField); // Recalculate total price
        });
        ...
        $(".decrease-qty").click(function() {
            ...
            if (parseInt(inputField.val()) > 0) {
                ...
                if (parseInt(inputField.val()) == 0) {
                    inputField.val(1);
                    toastr.warning('Quantity can not be zero.');
                }
                handleQuantityChange(inputField); // Recalculate total price
            }
        });
