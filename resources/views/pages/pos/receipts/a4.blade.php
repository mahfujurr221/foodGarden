            ...
            <div class="col-md-7 card card-body print">
                <div id="print-area">
                    ...
                    <div class="name">
                        ...
                        <span class="pull-right">Shop Name : {{ $pos->customer ? $pos->customer->shop_name : 'Walk-in Customer' }}</span>
                    </div>
                    <div class="address">
                        Address : <span>{{ $pos->customer ? $pos->customer->address->name : 'Walk-in Customer' }}</span>
                    </div>
                    <div class="mobile-no">
                    ...
                    <table class="table table-bordered table-plist my-3 order-details">
                        ...
                        @foreach ($pos->items as $key => $item)
                            <tr>
                                ...
                                <td>
                                    {{-- @if ($item->main_unit_qty)
                                        {{ $item->main_unit_qty }} {{ $item->product->main_unit->name }}
                                    @endif
                                    @if ($item->sub_unit_qty)
                                        {{ $item->sub_unit_qty }}
                                        {{ $item->product->sub_unit ? $item->product->sub_unit->name : '' }}
                                    @endif --}}
                            ...
                            <td>
                                <strong>
                                    ...
                                    @elseif(strpos($pos->discount, '%'))
                                        {{ number_format($pos->items->sum('sub_total') * (str_replace('%', '', $pos->discount) / 100), 2) }}
                                        Tk
                    ...
                    @endphp
                    <p class="in_word"><strong>In Word: </strong> {{ ucwords($digit->format($pos->receivable)) }} Taka only --}}
                    </p>
                    <p class="note">Note: {{ $pos->note }}</p>
                    <div class="signature">
