    ...
    <div class="card print_area">
        ...
        <div class="card-body card-body-soft p-4">
            ...
            <div class="table-responsive">
                <table class="table table-soft table-bordered" {{-- data-provide="datatables" --}}>
                    <thead>
                        <tr class="bg-primary">
                            <th>#</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ number_format($supplier->paid()) }} /-</td>
                            ...
                            <td style="text-align:center;">
                                ...
                                @endphp
                                <span style="font-weight: bold; font-size:1.3em;">{{ $wallet_balance }} /-</span>
                                <br>
                                @if($wallet_balance>0)
                                <p>** সাপ্লাইয়ারের কাছে আপনার টাকা জমা আছে</p>
                                @elseif($wallet_balance<0) <p>** সাপ্লাইয়ার আপনাকে দিয়েছে</p>
                                    @endif
                            </td>
                            <td class="font-weight-bold">
                                @php
                                $total_due = $supplier->due();
                                $wallet_balance < 0 ? $total_due +=abs($wallet_balance) : 0; @endphp {{
                                    number_format($total_due, 2) }} /- </td>

                            <td>
                                <form action="{{ route('supplier.update-status', $supplier->id) }}" method="post">
                                    @csrf
                                    <select name="status" class="form-control" onchange="this.form.submit()" required>
                                        <option value="1" {{ (int) $supplier->status === 1 ? 'selected' : '' }}>Active
                                        </option>
                                        <option class="text-red" value="0" {{ (int) $supplier->status === 0 ? 'selected' : '' }}>Inactive
                                        </option>
                                    </select>
                                </form>
                            </td>
                            ...
                            <td class="print_hidden">
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" x-placement="bottom-start">
                                        ...
                                        @endif
                                        <a href="{{ route('supplier.wallet_payment',$supplier->id) }}" class="edit
                                            dropdown-item" data-toggle="modal" data-target="#edit" id="Wallet Payment">
                                            <i class="fa fa-money text-primary"></i>
                                            Wallet Payment
                                        </a>
                                        <a class="dropdown-item" href="{{ route('supplier.report', $supplier->id) }}">
                                            <i class="fa fa-file-excel-o"></i>
                                            Report
                                        </a>
                                        <a href="{{ route('report.supplier_ledger') }}?supplier_id={{ $supplier->id }}"
                                            class="dropdown-item">
                                            <i class="fa fa-book"></i>
                                            Ledger
                                        </a>

                                        <a href="{{ route('purchase.index') }}?supplier={{ $supplier->id }}"
                                            class="dropdown-item" target="_blank">
                                            <i class="fa fa-list"></i>
                                            Purchase List
                                        </a>

                                        <a href="{{ route('payment.index') }}?supplier={{ $supplier->id }}"
                                            class="dropdown-item" target="_blank">
