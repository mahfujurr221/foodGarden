            ...
            <div id="print-area">
                <div class="invoice-header row">
                    ...
                    <div class="invoice col-4">
                        ...
                        <p><strong>Shop Name :</strong> {{ $estimate->customer ? $estimate->customer->shop_name_bangla :
                            'Walk-in Customer' }}</p>
                        <p><strong>Customer Name :</strong> {{ $estimate->customer ? $estimate->customer->name :
                            'Walk-in Customer' }}</p>
                        <p><strong>Address</strong> {{ $estimate->customer ? $estimate->customer->address->name :
                            'Walk-in Customer'}}</p>
                        <p><strong>Mobile : </strong>{{ $estimate->customer ? $estimate->customer->phone : 'Walk-in
                            Customer'}}</p>
                    </div>
                ...
                <table class="table" style="margin-bottom:0px !important;">
                    <tr>
                        <td><strong>INVOICE : {{ $estimate->id }}</td></strong>
                        <td class="text-right"><strong>DATE : {{ date('d/m/ Y', strtotime($estimate->estimate_date)) }}</td>
                        </strong>
                    </tr>
                </table>
                ...
                <table class="table table-bordered table-plist mb-3 order-details" style="margin-top:0px !important;">
                    ...
                    @php
                    $previousDue = $estimate->customer ? bcsub($estimate->customer->receivable() , $estimate->customer->paid()) : 0;
                    $wallet_due=$estimate->customer?$estimate->customer->wallet_balance():0;
                    if($wallet_due<0){ $previousDue+=abs($wallet_due); }
                    ...
                    {{--<tr>
                        ...
                        <td>
                            <strong>{{ number_format($estimate->paid,2) }} </strong>Tk
                        </td>
                    </tr>--}}
                    ...
                    {{-- <tr>
                        ...
                        <td>
                            <strong>{{ round($estimate->delivery_cost) }} </strong>Tk
                        </td>
                    </tr> --}}

                    {{-- @if($estimate->previous_returned_product_value()>0)
                    <tr>
                        <td colspan="4" class="rm-b-l rm-b-t rm-b-b"></td>
                        <td class="text-right">Previous Returned : </td>
                        <td>
                            <strong>{{ number_format($estimate->previous_returned_product_value(),2) }} </strong>Tk
                        </td>
                    </tr>
                    @endif --}}
                ...
                <div class="footer-note">
                    ...
                    @endphp
                    <p class="in_word mb-0">Tk. In Word: {{ ucwords($digit->format($estimate->receivable)) }} Taka only
                    </p> --}}
                    <p class="note">Notes: {{ $estimate->note }}</p>
                </div>
                <div class="signature">
