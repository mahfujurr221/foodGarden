<script>
        ...
        select: function(event, ui) {
            ...
            $.get(url, (product) => {
                ...
                if (pExist(product.id) == true) {
                    ...
                    var field = $(selector);
                    var main_qty = parseInt(empty_field_check(field.parents('tr').find('.main_qty')
                        .val()));
        ...
        if (product.sub_unit == null) {
            ...
            quantity_data =
                `<input type="text" class="has_sub_unit" hidden value="true">
                    ...
                    <input type="number" placeholder="${product.main_unit.name}" value="1" class="form-control col main_qty" name="main_qty[${product.id}]" data-value="${product.stock}" data-related="${product.main_unit.related_by}" onkeydown="return event.keyCode !== 190" min="0">
                    <input type="number" placeholder="${product.sub_unit.name}" value="" class="form-control col sub_qty" name="sub_qty[${product.id}]"  onkeydown="return event.keyCode !== 190" min="0" max="${product.main_unit.related_by-1}">`;
        }
        ...
        let dom = `
              <tr class="background_gray">
                ...
                <td style="display:none;">
                    <input type="text" value="0" class="form-control discount_qty" name="discount_qty[${product.id}]">
                </td>
                ...
                <td>
                  <input type="number" name="sub_total[${product.id}]" class="form-control sub_total border_none text-center" value="${Math.round(product.price)}"/>
                </td>

                <td style="background: red; text-align: center; cursor: pointer; color:white;">
                  <a href="#" class="remove-btn item-index" data-value="${index}">
                    <i class="fa fa-trash" style="font-size: 10px; padding:0px;" ></i>
                    </a>
                </td>
              </tr>
         `;
    ...
    $("#order-btn").on('click', function(e) {
        ...
        if (customerId == 0 && due < 0) {
            e.preventDefault();
            toastr.warning(
                'Walk-in Customer is do not support due. Please make Payment or Change Customer');
        } else if (due > 0) {
            e.preventDefault();
